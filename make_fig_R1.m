...
%% datasets
ALL_FOLDER_FILES = {'P:\server\P1\sign_analysis\our_data\th1\', 'P:\server\P1\sign_analysis\our_data\th1_p4\',...
    'P:\server\P1\sign_analysis\external_data\Bonneau\hiv\', 'P:\server\P1\sign_analysis\external_data\Bonneau\mock\'};
...
for n_data = 1:n_dataset
    ...
    %% load
    if ~(contains(FOLDER_FILES, 'th0') || contains(FOLDER_FILES, 'th17'))
        %data, all replicates
        ...
        Xtarget = importdata([FOLDER_FILES, 'X.mat']); Ntarget = size(Xtarget,1);
        Xtf = importdata([FOLDER_FILES, 'Xtf.mat']); Ntf = size(Xtf,1);
        Xatac = importdata([FOLDER_FILES, 'Xatac.mat']); Natac = size(Xatac,1);
    end
    NTP = size(Xtarget,2);
    ...
    elseif contains(FOLDER_FILES, 'hiv') || contains(FOLDER_FILES, 'mock')
        ...
        ORG = "human";
    elseif contains(FOLDER_FILES, 'th0')
        time = [3,6,16]; %th0
        ORG = "mouse";
    elseif contains(FOLDER_FILES, 'th17')
        time = [3,6,16,48]; %th17
        ORG = "mouse";
    else
        error()
    ...
    fprintf('\t Data: %.f%% FFLs are coherent\n', prc_cohe_FFL(n_data))
    ...
    %can be approximated to a guassian, as can be seen from plot below
    [f,x] = ksdensity(prc_cohe_vec,'Function','pdf');
    [f2,x2] = ksdensity(guass_vec,'Function','pdf');
    figure 
    hold on
    plot(x,f)
    plot(x2,f2)
    xlabel('% coherent FFL')
    legend('Montecarlo pdf','Gaussian pdf')
...
%     
% %     %percentage of coherent
% % 	pc_zero_cohe = arrayfun(@(r) test_partial_correlation(r, NTP, 1), Rtga(is_cohe)); 
% % 	pc_zero_inco = arrayfun(@(r) test_partial_correlation(r, NTP, 1), Rtga(~is_cohe)); 
% %     
% % 	fprintf('\t In coherent FFLs %d/%d=%.2f%% R(T,G|A) are c.i.\n', ...
% %         sum(pc_zero_cohe), numel(pc_zero_cohe), 100*sum(pc_zero_cohe)/numel(pc_zero_cohe))
% % 	fprintf('\t In incoherent FFLs: %d/%d=%.2f%% R(T,G|A) are c.i.\n', ...
% %         sum(pc_zero_inco), numel(pc_zero_inco), 100*sum(pc_zero_inco)/numel(pc_zero_inco))
%    
% 	%plot
% 	figure
...
%     
% %     %percentage of coherent
% % 	pc_not_zero_cohe = arrayfun(@(r) test_partial_correlation(r, NTP, 1), Ragt(is_cohe))'; 
% % 	pc_not_zero_inco = arrayfun(@(r) test_partial_correlation(r, NTP, 1), Ragt(~is_cohe))'; 
% %     
% % 	fprintf('\t In coherent FFLs %d/%d=%.2f%% R(A,G|T) are c.i.\n', ...
% %         sum(pc_not_zero_cohe), numel(pc_not_zero_cohe), 100*sum(pc_not_zero_cohe)/numel(pc_not_zero_cohe))
% % 	fprintf('\t In incoherent FFLs: %d/%d=%.2f%% R(A,G|T) are c.i.\n', ...
% %         sum(pc_not_zero_inco), numel(pc_not_zero_inco), 100*sum(pc_not_zero_inco)/numel(pc_not_zero_inco))    
%     
%     %plot
% 	figure
...
%         Rxy(iter) = dyncorr( X(iter,:)', Y(iter,:)', time);
%         %PC
%         Rxw(iter) = dyncorr( X(iter,:)', W(iter,:)', time);
%         Rwy(iter) = dyncorr( Y(iter,:)', W(iter,:)', time);
%         Rxy_w(iter) = pc_fun( Rxy(iter), Rxw(iter), Rwy(iter));
%         %double PC
%         Rxz(iter) = dyncorr( X(iter,:)', Z(iter,:)', time);
%         Ryz(iter) = dyncorr( Y(iter,:)', Z(iter,:)', time);
%         Rwz(iter) = dyncorr( W(iter,:)', Z(iter,:)', time);
%        
%         Rxz_w(iter) = pc_fun( Rxz(iter), Rxw(iter), Rwz(iter));
%         Ryz_w(iter) = pc_fun( Ryz(iter), Rwy(iter), Rwz(iter));
% 
%         Rxy_wz(iter) = pc_fun(Rxy_w(iter), Rxz_w(iter), Ryz_w(iter)); 
%         
%         %coherence
%         FFL1 = Rxz(iter)*Rxw(iter)*Rwz(iter)>0;
%         FFL2 = Ryz(iter)*Rwy(iter)*Rwz(iter)>0;
%         fork = Rxy(iter)*Rxz(iter)*Ryz(iter)>0;
%         collider = Rxy(iter)*Rxw(iter)*Rwz(iter)>0;
%         is_cohe(iter) = fork;
%         
%     end
%        
%     figure
%     subplot(1,2,1)
% 	print_density({Rxy_w(is_cohe),Rxy_w(~is_cohe)},'R(X,Y$\vert$W)',['IMG/Rxy_w_',FIG_NAME])
%     subplot(1,2,2)
% 	print_density({Rxy_wz(is_cohe),Rxy_wz(~is_cohe)},'R(X,Y$\vert$W,Z)',['IMG/Rxy_wz_',FIG_NAME])
